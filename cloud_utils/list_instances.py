...
def get_instace_object_from_gcp_list(project, raw, region_to_instances):
  ...
  for region, region_instances in region_to_instances.iteritems():
    for matching_instance in region_instances.get('instances', []):
      created_by = [item['value'] for item in matching_instance.get('metadata', {}).get('items', []) if item['key'] == 'created-by']
      ...
      matching_intsances.append(Instance(cloud='gcp',
                                         ...
                                         private_ip_address=matching_instance['networkInterfaces'][0].get('networkIP'),
                                         project=project,
                                         security_groups=[],
                                         tags=tags,
...
def get_volumes(instance):
  ...
  if instance.cloud == 'aws':
    client = boto3.client('ec2', instance.region)
    devices = client.describe_instance_attribute(InstanceId=instance.id, Attribute='blockDeviceMapping').get('BlockDeviceMappings', [])
